...
Module Make (Entries:BUILD_ENTRIES).
 ...
 Definition PPT_add_adv_infob : forall E1 E t (A:Proc.proc t) PrOrcl PrPriv
  ...
  intros E1 E t A PrOrcl PrPriv Heq HA pi.
  case_eq (PrSet.forallb (fun f => res_poly E tsize_limit (BProc.p_name f)) PrOrcl);
  [intro | intro; exact pi].
  case_eq (PrSet.forallb (fun f => cmd_poly pi (proc_body E (BProc.p_name f))) PrOrcl);
  [intro | intro; exact pi].
  case_eq (PrSet.forallb (fun f => expr_poly (proc_res E (BProc.p_name f))) PrOrcl);
  [intro | intro; exact pi].
  ...
  refine (PrSet.forallb_correct _ _ PrOrcl H _ H4).
  intros x y; unfold PrSet.E.eq; intro Hxy; rewrite Hxy; trivial.
  refine (cmd_poly_spec pi _ _).
  refine (PrSet.forallb_correct _ _ PrOrcl H0 _ H4).
  intros x y; unfold PrSet.E.eq; intro Hxy; rewrite Hxy; trivial.
  refine (expr_poly_spec _ _).
  refine (PrSet.forallb_correct _ _ PrOrcl H1 _ H4).
  intros x y; unfold PrSet.E.eq; intro Hxy; rewrite Hxy; trivial.
