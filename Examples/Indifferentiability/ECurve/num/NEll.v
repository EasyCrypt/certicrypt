...
Record ex_spec (exx: ex): Prop := mkExS {
  n2_div: ~(2 | Zpos exx.(vN));
  n_pos: 2 < Zpos exx.(vN);
 lprime: 
  forall p : positive * positive, List.In p (vR exx) -> prime (Zpos (fst p));
   lbig:
    4 * Zpos (vN exx) < (Zpos (Zmullp (vR exx)) - 1) ^ 2;
   inC:
    vy exx ^ 2 mod (Zpos (vN exx)) = (vx exx ^ 3 + vA exx * vx exx + vB exx) mod (Zpos (vN exx))
}.
...
Variable N_small:  Zpos exx.(vN) < base (znz_digits op).

Lemma z2ZN: z2Z zN = Zpos exx.(vN).
apply (@znz_of_Z_correct _ _ op_spec (Zpos exx.(vN))); split; auto with zarith.
Qed.

Definition Z2z z :=
 match z mod (Zpos exx.(vN)) with
...
Lemma nplusz: forall x y, wft x -> wft y -> 
      z2Z (x +++ y) = ZEll.nplus (Zpos (vN exx)) (z2Z x) (z2Z y).
Proof.
intros x y Hx Hy.
unfold z2Z, nplus.
...
Lemma nsubz: forall x y, wft x -> wft y -> 
      z2Z (x -- y) = ZEll.nsub (Zpos (vN exx)) (z2Z x) (z2Z y).
Proof.
intros x y Hx Hy.
unfold z2Z, nsub.
...
Lemma nmulz: forall x y, wft x -> wft y -> 
      z2Z (x ** y) = ZEll.nmul (Zpos (vN exx)) (z2Z x) (z2Z y).
Proof.
intros x y Hx Hy.
unfold z2Z, nmul.
...
Lemma z2Zx: forall x, z2Z (Z2z x) = x mod (Zpos exx.(vN)).
unfold Z2z; intros x.
generalize (Z_mod_lt x (Zpos exx.(vN))).
case_eq (x mod (Zpos exx.(vN))).
intros _ _.
...
apply Zmult_gt_0_le_compat_r; auto with zarith.
  ...
  case (op_spec.(spec_to_Z) (snd v)); auto with zarith.
intros p Hp; case (Z_mod_lt x (Zpos exx.(vN))); auto with zarith.
...
Lemma c2ww: forall x, ZEll.nmul (Zpos (vN exx)) 2 x = ZEll.nmul (Zpos (vN exx)) (z2Z c2) x.
intros x; unfold ZEll.nmul.
unfold c2; rewrite z2Zx; rewrite Zmodml; auto.
Qed.

Lemma c3ww: forall x, ZEll.nmul (Zpos (vN exx)) 3 x = ZEll.nmul (Zpos (vN exx)) (z2Z c3) x.
intros x; unfold ZEll.nmul.
unfold c3; rewrite z2Zx; rewrite Zmodml; auto.
Qed.

Lemma Aww: forall x, ZEll.nmul (Zpos (vN exx)) exx.(vA) x = ZEll.nmul (Zpos (vN exx)) (z2Z A) x.
intros x; unfold ZEll.nmul.
...
Lemma nadd_correct: forall x y sc,
  wfe x -> wfe y -> wft sc ->
  e2E (fst (nadd sc x y)) = fst (ZEll.nadd (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) (e2E y) )/\
  z2Z (snd (nadd sc x y)) = snd (ZEll.nadd (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) (e2E y)).
Proof.
intros x; case x; clear; auto.
intros x1 y1 z1 y; case y; clear; auto.
...
Lemma ndouble_correct: forall x sc,
  wfe x -> wft sc ->
  e2E (fst (ndouble sc x)) = fst (ZEll.ndouble (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x))/\
  z2Z (snd (ndouble sc x)) = snd (ZEll.ndouble (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x)).
Proof.
intros x; case x; clear; auto.
...
Lemma nopp_correct: forall x,
  wfe x -> e2E x = ZEll.nopp (Zpos exx.(vN)) (e2E (nopp x)).
Proof.
...
apply sym_equal.
rewrite <- (Z_mod_plus) with (b := -(-z2Z y1 / (Zpos exx.(vN)))); auto with zarith.
...
Lemma scalb_correct: forall n b x sc,
  wfe x -> wft sc ->
  e2E (fst (scalb sc b x n)) = fst (ZEll.scalb (Zpos exx.(vN)) exx.(vA) (z2Z sc) b (e2E x) n)/\
  z2Z (snd (scalb sc b x n)) = snd (ZEll.scalb (Zpos exx.(vN)) exx.(vA) (z2Z sc) b (e2E x) n).
Proof.
...
intros p Hrec b x sc H1 H2.
  case b; unfold scalb; fold scalb.
    ...
    replace (ZEll.nopp (Zpos (vN exx)) (e2E x)) with
      (e2E (nopp x)).
...
Lemma scal_correct: forall n x sc,
  wfe x -> wft sc ->
  e2E (fst (scal sc x n)) = fst (ZEll.scal (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) n)/\
  z2Z (snd (scal sc x n)) = snd (ZEll.scal (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) n).
Proof.
...
Lemma scal_list_correct: forall l x sc,
  wfe x -> wft sc ->
  e2E (fst (scal_list sc x l)) = fst (ZEll.scal_list (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) l)/\
  z2Z (snd (scal_list sc x l)) = snd (ZEll.scal_list (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) l).
Proof.
...
Lemma scalL_correct: forall l x sc,
  wfe x -> wft sc ->
  e2E (fst (scalL sc x l)) = fst (ZEll.scalL (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) l)/\
  z2Z (snd (scalL sc x l)) = snd (ZEll.scalL (Zpos exx.(vN)) exx.(vA) (z2Z sc) (e2E x) l).
Proof.
intros l1; elim l1; simpl; auto.
intros a l2 Hrec x sc H1 H2.
...
Lemma mww: forall x y, ZEll.nmul (Zpos (vN exx)) (x mod (Zpos (vN exx)) ) y = ZEll.nmul (Zpos (vN exx)) x y.
intros x  y; unfold ZEll.nmul; rewrite Zmodml; auto.
Qed.

Lemma wwA: forall x, ZEll.nmul (Zpos (vN exx)) x exx.(vA) = ZEll.nmul (Zpos (vN exx)) x (z2Z A).
intros x; unfold ZEll.nmul.
unfold A; rewrite z2Zx; rewrite Zmodmr; auto.
Qed.

Lemma wwB: forall x, ZEll.nmul (Zpos (vN exx)) x exx.(vB) = ZEll.nmul (Zpos (vN exx)) x (z2Z B).
intros x; unfold ZEll.nmul.
...
Qed.
 ...
 Lemma  scalL_prime: 
  ...
  let (a3, sc3) := scalL sc2 a2 R1 in
    match a3 with
     nzero => if (Zeq_bool (Zgcd (z2Z sc3) (Zpos exx.(vN))) 1) then prime (Zpos exx.(vN))
              else True
   | _ => True
   end.
  Proof.
  ...
  intros Ha3; case_eq (Zeq_bool (Zgcd (z2Z sc3) (Zpos (vN exx))) 1); auto.
  intros H1.
  assert (F0: 
     (vy exx mod (Zpos (vN exx))) ^ 2 mod (Zpos (vN exx)) =
       ((vx exx mod (Zpos (vN exx))) ^ 3 + vA exx * (vx exx mod (Zpos (vN exx))) +
        vB exx) mod (Zpos (vN exx))).
      generalize exxs.(inC).
      ...
      apply f_equal2 with (f := Zmod); auto.
      rewrite Zmod_mod; auto.
   generalize (@ZEll.scalL_prime (Zpos exx.(vN)) 
               (exx.(vx) mod (Zpos exx.(vN)))
               (exx.(vy) mod (Zpos exx.(vN)))
               exx.(vA)
               exx.(vB) 
...
replace (e2E a) with (ZEll.ntriple (vx exx mod (Zpos (vN exx))) (vy exx mod (Zpos (vN exx))) 1).
case ZEll.scal.
...
Lemma isM2_correct: forall p,
  if isM2 p then ~(2| Zpos p) /\ 2 < Zpos p else True.
Proof.
...
Definition ell_test (N S: positive) (l: List.list (positive * positive))
  ...
  let mop := make_mod_op op (znz_of_Z op (Zpos N)) in
    if isM2 N then
    match (4 * (Zpos N)) ?= (Zpos (ZEll.Zmullp l) - 1) ^ 2  with
      Lt => 
       match y ^ 2 mod (Zpos N) ?= (x ^ 3 + A * x + B) mod (Zpos N) with
       Eq => 
          ...
          match a3 with
           nzero => if (Zeq_bool (Zgcd (z2Z op sc3) (Zpos N)) 1) then true
                    else false
          | _ => false
          end
      | _  => false
...
Lemma ell_test_correct: forall (N S: positive) (l: List.list (positive * positive))
                      (A B x y: Z),
  (forall p, List.In p l -> prime (Zpos (fst p))) -> 
  if ell_test N S l A B x y then prime (Zpos N) else True.
...
set (op := cmk_op n).
set (mop := make_mod_op op (znz_of_Z op (Zpos N))).
set (exx := mkEx N S1 l A1 B1 x y).
set (op_spec := cmk_spec n).
assert (exxs: ex_spec exx).
  constructor; auto.
assert (H0: (Zpos N) < base (znz_digits op)).
