...
Definition gpow n := 
  match n with  
  | Z0 => G.(e) 
  | Zpos p => iter_pos p _ (op a) G.(e)  
  | Zneg p  => G.(i) (iter_pos p _ (op a) G.(e))
  end.
...
Proof.
 ...
 intro p; apply i_internal.
 apply iter_pos_invariant with (Inv := fun x => In x G.(s)); auto.
Qed.
...
Lemma gpow_neg : forall p, gpow (Zneg p) = i G (gpow (Zpos p)).
Proof.
 intros p;simpl; trivial.
Qed.

Lemma gpow_add_pos_neg :forall p1 p2 : positive,
   gpow (Zpos p1 + Zneg p2) = op (gpow (Zpos p1)) (gpow (Zneg p2)).
Proof.
 intros p1 p2; case_eq (Zpos p1 + Zneg p2); intros.
 
 assert (- Zpos p1 = Zneg p2) by omega.
 rewrite <- H0.
 cutrewrite (- Zpos p1 = Zneg p1); simpl; trivial.
 rewrite i_is_inverse_r; trivial.
 exact (gpow_in (Zpos p1)).

 assert (Zpos p1 = Zpos p + Zpos p2).
 rewrite <- H, <- (Zopp_neg p2).
 cutrewrite (Zpos p1 + Zneg p2 + - Zneg p2 = Zpos p1);[ trivial | ring ].
 rewrite H0, gpow_add; simpl.
 rewrite <- (assoc G), i_is_inverse_r.
 apply gpow_op.
 apply e_in_s.
 exact (gpow_in (Zpos p2)).
 exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p2)).
 apply i_internal. 
 exact (gpow_in (Zpos p2)).
 apply Zle_0_pos.
 ...
 assert (Zpos p2 = Zpos p + Zpos p1).
 cutrewrite (Zpos p = Zpos p2 + Zneg p1).
 rewrite <- (Zopp_neg p1); ring.
 rewrite <- (Zopp_neg p2), <- (Zopp_neg p).
 cutrewrite (Zneg p1 = - Zpos p1); [omega | ].
 rewrite <- (Zopp_neg p1); ring.
 repeat rewrite gpow_neg.
 rewrite H0, gpow_add, inv_op, (assoc G), i_is_inverse_r, e_is_zero_l; trivial.
 apply i_internal; exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p1)).
 exact (gpow_in (Zpos p1)).
 apply i_internal; exact (gpow_in (Zpos p1)).
 apply i_internal; exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p1)).
 apply Zle_0_pos.
...
Lemma gpow_add_neg_pos :forall p1 p2 : positive,
   gpow (Zneg p1 + Zpos p2) = op (gpow (Zneg p1)) (gpow (Zpos p2)).
Proof.
 intros p1 p2; case_eq (Zneg p1 + Zpos p2); intros.
 
 assert (- Zneg p1 = Zpos p2) by omega.
 rewrite <- H0.
 cutrewrite (- Zneg p1 = Zpos p1); simpl; trivial.
 rewrite i_is_inverse_l; trivial.
 exact (gpow_in (Zpos p1)).
 ...
 rewrite <- (Zopp_neg p1); ring.
 repeat rewrite gpow_neg.
 rewrite H0, Zplus_comm, gpow_add, (assoc G), i_is_inverse_l, e_is_zero_l; trivial.
 exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p1)).
 apply i_internal; exact (gpow_in (Zpos p1)).
 exact (gpow_in (Zpos p1)).
 ...
 assert (Zpos p1 = Zpos p + Zpos p2).
 cutrewrite (Zpos p = Zpos p1 + Zneg p2).
 rewrite <- (Zopp_neg p2); ring.
 rewrite <- (Zopp_neg p1), <- (Zopp_neg p).
 cutrewrite (Zneg p2 = - Zpos p2); [omega | ].
 rewrite <- (Zopp_neg p2); ring.
 repeat rewrite gpow_neg.
 rewrite H0, Zplus_comm, gpow_add, inv_op, <- (assoc G), i_is_inverse_l, e_is_zero_r; trivial.
 apply i_internal; exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p2)).
 apply i_internal; exact (gpow_in (Zpos p)).
 apply i_internal; exact (gpow_in (Zpos p2)).
 exact (gpow_in (Zpos p2)).
 exact (gpow_in (Zpos p2)).
 exact (gpow_in (Zpos p)).
 apply Zle_0_pos.
...
Theorem gpow_add_full: forall n m, gpow (n + m) = op (gpow n) (gpow m).
Proof.
 intros n; case n.
 
 intros m; simpl; apply sym_equal; apply e_is_zero_l; apply gpow_in.
 
 intros p1 m; case m; intros.
 simpl; apply sym_equal; apply e_is_zero_r.
 exact (gpow_in (Zpos p1)).
 apply gpow_add; apply Zle_0_pos.
 apply gpow_add_pos_neg.
 
 intros p1 m; case m; intros.
 simpl; apply sym_equal; apply e_is_zero_r.
 apply i_internal.
 exact (gpow_in (Zpos p1)).
 apply gpow_add_neg_pos.
 ...
 rewrite <- inv_op. 
 apply f_equal.
 rewrite <- gpow_op, Pplus_comm, iter_pos_plus; trivial.
 exact (gpow_in (Zpos p1)).
 exact (gpow_in (Zpos p)).
 exact (gpow_in (Zpos p1)).
Qed.
...
Theorem gpow_gpow: forall (A : Type) (op : A -> A -> A) (a : A) (G : FGroup op),
...
Proof.
 ...
 intros.
 assert (H2 := Zlt_neg_0 p).
 elimtype False; omega.
Qed.

Theorem gpow_e: forall (A : Type) (op : A -> A -> A) (G : FGroup op) n, 0 <= n -> gpow G.(e) G n = G.(e).
 intros A op G n; case n; simpl; auto with zarith.
 intros p _; elim p; simpl; auto; intros p1 Rec; repeat rewrite Rec; auto.
 intros; assert (H2 := Zlt_neg_0 p).
 elimtype False; omega.
Qed.

Theorem gpow_pow: forall (A : Type) (op : A -> A -> A) (a : A) (G : FGroup op),
...
Theorem gpow_mult: forall (A : Type) (op : A -> A -> A) (a b: A) (G : FGroup op)
...
Theorem order_mult: forall (A : Type) (op : A -> A -> A) (A_dec: forall a b: A, {a = b} + {~ a = b}) (G : FGroup op)
...
Theorem fermat_gen: forall (A : Type) (A_dec: forall (a b: A), {a = b} + {a <>b}) (op : A -> A -> A) (a: A) (G : FGroup op),
...
Theorem order_div: forall (A : Type) (A_dec: forall (a b: A), {a = b} + {a <>b}) (op : A -> A -> A) (a: A) (G : FGroup op) m,
