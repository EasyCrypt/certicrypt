...
Section Nell.  
 ...
 Lemma Zmull_div : forall a l, List.In a l ->
    (Zpos (Zmull l)  = (Zpos (Zmull l) / Zpos a) * Zpos a)%Z.
 ...
 Lemma psplit_prime: forall l (p: positive),
  (forall (q: positive * positive), List.In q l -> prime (Zpos (fst q))) ->
   prime (Zpos p) -> (Zpos p | Zpos (Zmullp l)) -> List.In p (snd (psplit l)).
 Proof.
 intros l; elim l.
   unfold Zmullp; simpl.
   intros p _ H1 H2.
...
Axiom ok: forall P, P.
 ...
 Lemma scalb_correct: forall p sc b a1 p1,
   a1 =~~= p1 ->  [[scalb sc b a1 p]] -> 
     fst (scalb sc b a1 p) =~~= 
     EGroup.gpow p1 pG (if b then Zpos (Psucc p) else Zpos p).
 Proof.
 ...
 intros p0; unfold scalb; elim p0; clear p0; fold scalb.
   intros p0 Hrec sc b a1 p1; case b; clear b; intros H1.
     ...
     replace (gpow p1 pG (Zpos p0 + Zpos p0 + 1)) with
             (add (opp p1) (gpow p1 pG (Zpos (Psucc p0) + Zpos (Psucc p0)))).
     apply H6; auto.
   ...
   intros p0 Hrec sc b a1 p1; case b; clear b; intros H1.
     ...
     simpl Psucc.
     rewrite ZCmisc.Zpos_xI_add.
     replace (Zpos p0 + Zpos p0 + 1)%Z with
             (1 + (Zpos p0 + Zpos p0))%Z; auto with zarith.
     repeat rewrite gpow_add; auto with zarith.
     repeat rewrite gpow_1; auto with zarith.
     apply H6; auto.
     apply H4; auto.
 ...
 Lemma scal_correct: forall p sc a1 p1,
   a1 =~~= p1 ->  [[scal sc a1 p]] -> 
     fst (scal sc a1 p) =~~= EGroup.gpow p1 pG (Zpos p).
 Proof.
 ...
 Lemma scal_list_correct: forall l sc a1 p1,
   a1 =~~= p1 ->  [[scal_list sc a1 l]] -> 
     fst (scal_list sc a1 l) =~~= EGroup.gpow p1 pG (Zpos (Zmull l)).
 Proof.
 assert (F0: forall p1, List.In p1 (FGroup.s pG)).
    intros p1.
    apply (FELLK_in pell_theory _ (in_all_znz _ p_pos)).
 ...
 change (equiv (fst (scal_list sc2 a2 l)) (gpow (gpow p1 pG (Zpos a)) pG (Zpos (Zmull l)))).
 apply Hrec.
 ...
 Lemma scalL_not_1: forall l sc a p1 n,
    ...
    gpow p1  pG (Zpos (Zmull l) / Zpos n) <> pG.(FGroup.e).
 Proof. 
 assert (F0: forall p1, List.In p1 (FGroup.s pG)).
    intros p1.
    apply (FELLK_in pell_theory _ (in_all_znz _ p_pos)).
 ...
 Lemma  scalL_1: forall l sc a p1,
  a =~~= p1 -> [[scalL sc a l]] -> 
  fst (scalL sc a l) =~~= gpow p1  pG (Zpos (Zmull l)).
 Proof.
 assert (F0: forall p1, List.In p1 (FGroup.s pG)).
    intros p1.
    apply (FELLK_in pell_theory _ (in_all_znz _ p_pos)).
...
Section pell.  
 ...
 Hypothesis lR_prime: forall p, List.In p lR -> prime (Zpos (fst p)).
 Variable F: positive.
 Hypothesis lR_big: (4 * N < (Zpos (Zmullp lR) -1) ^2)%Z.
 ...
 Lemma  scalL_prime: 
  ...
  assert (E1: pequiv a (pcurve_elt (to_p x) (to_p y) Hoc)).
    ...
    assert (E2: e_order (ceqb pell) p1 ppG = Zpos (Zmullp lR)).
      ...
      intros z1 Hz1 Hz2.
       ...
       set (p2 := gpow (gpow p1 ppG (Zpos S1)) ppG (Zpos (Zmull R1))).
    ...
    absurd (e_order(ceqb pell) p1 ppG <= (FGroup.g_order ppG)).
      ...
      rewrite E2.
      replace (Zpos (Zmullp lR)) with (( Zpos (Zmullp lR) - 1) + 1)%Z; try ring.
