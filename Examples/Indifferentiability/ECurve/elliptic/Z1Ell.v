...
Section Nell.  
 ...
 Definition ndouble := fun (p1: nelt) =>
 match p1 with
 | (ntriple x1 y1 z1) =>
    if (y1 ?= 0) then (ntriple 0 1 0) else
     (* we do 2p *)
 ...
 Definition nadd := fun (p1 p2: nelt) =>
 match p1, p2 with
 ...
 Inductive nInversible: nelt -> Prop :=
 |nInv_e: forall x y z,
     inversible N z -> nInversible (ntriple x y z).
 ...
 Lemma nInversible_double: forall x,
  [[ndouble x]] -> [[x]].
 Proof.
 unfold ndouble; intros x; case x; auto.
 intros x1 y1 z1; case Zeq_bool; auto.
   intros H; inversion_clear H.
   case inversible_0; auto.
 intros H; inversion_clear H; auto.
constructor; repeat itac.
 Qed.

 Lemma nInversible_add: forall x y,
  [[nadd x y]] -> [[x]].
 Proof.
 unfold nadd; intros x y; case x; intros x1 y1 z1;
   case y; intros x2 y2 z2; case Zeq_bool.
  case Zeq_bool.
    case Zeq_bool; intros H; inversion_clear H.
      case inversible_0; auto. 
    constructor; repeat itac.
  intros H; inversion_clear H; case inversible_0; auto.
  intros H; inversion_clear H; constructor; repeat itac.
Qed.

 Fixpoint scal (a: nelt) (p: positive) {struct p}:
   nelt :=
   match p with
     xH => a
   | xO p1 => ndouble (scal a p1)
   | xI p1 => nadd a (ndouble (scal a p1)) 
   end.

 Lemma nInversible_scal: forall a1 p1,
  [[scal a1 p1]] -> [[a1]].
 Proof.
 intros a p0; generalize a; elim p0; simpl; auto;
    clear a p0. 
 intros p0 Hrec a Ha; apply nInversible_add with (1:= Ha).
 intros p0 Hrec a H1; apply Hrec; apply nInversible_double with (1:= H1).
 Qed.  

  Definition scal_list a l := List.fold_left scal l a.

 Lemma nInversible_scal_list: forall a l,
  [[scal_list a l]] -> [[a]].
 Proof.
 intros a l; generalize a; unfold scal; 
   elim l; simpl; auto;
    clear a l.
 intros n l Hrec a H1; apply nInversible_scal with n; auto. 
 Qed.  
